 ...
 */
class Ship_Order_Model implements JsonSerializable {

	private $_properties;

	private $_table = 'ship_orders';
    ...
    public static function instance($id = null) {
		...
		global $wpdb;
		$table = $wpdb->prefix . 'ship_orders';
		$object = $wpdb->get_row("SELECT * FROM $table WHERE id = $id", ARRAY_A);
		return new Ship_Order_Model($object);

	}

	public static function get_order_by($field, $value) {
		global $wpdb;
		$table = $wpdb->prefix . 'ship_orders';
		$object = $wpdb->get_row("SELECT * FROM $table WHERE $field = $value", ARRAY_A);
		return new Ship_Order_Model($object);
	}
